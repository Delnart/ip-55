...
from bot.utils.api import ScheduleAPI # Імпортуємо логіку розкладу

# Змінюємо app = FastAPI() на router = APIRouter()
router = APIRouter()
...
# Додаємо відсутній ендпоінт для розкладу
@router.get("/schedule")
...
async def get_queue_by_subject(subject_id: str):
    ...
    for entry in queue.get("entries", []):
        if entry.get("userId"):
            user = await db.db.users.find_one({"_id": entry["userId"]}) # Виправлено на db.db
            if user:
                entry["user"] = {
                    "_id": str(user["_id"]),
...
async def join_queue(entry: QueueEntry):
    try:
        queue = await db.db.queues.find_one({"_id": entry.queueId}) # Виправлено на db.db
        if not queue:
        ...
        user = await db.db.users.find_one({"telegramId": entry.telegramId}) # Виправлено на db.db
        if not user:
            raise HTTPException(status_code=404, detail="User not found")
